...
const std::string TYPE_FLOOR = "floor";
...
const GLfloat TIME_SPEED_INCREASE_SPEED = 1.01f;
const GLfloat EXPLOSION_SPEED = 128;
const GLfloat EXPLOSION_DESPAWN = 50.0;
const int FLOOR_SCALE = 2;
...
const int KEY_INCREASE_TIME_SPEED = GLFW_KEY_J;
const int KEY_DECREASE_TIME_SPEED = GLFW_KEY_K;
...
GLfloat mod_float(float a, float b) {
	return a - int(a / b) * b;
}
...
private:
	...
	public:
		...
		friend std::ofstream& operator<<(std::ofstream& ofs, const MyObject& obj) {
			...
			ofs << obj.explosion_rate << std::endl;
	...
	void update_single_object(const MyObject& obj) {
		...
		glUniform1f(glGetUniformLocation(obj.program, "explosion_rate"), obj.explosion_rate);
...
public:
	...
	void despawn_exploded() {
		for (size_t obj = 0; obj != objects.size(); ++obj) {
			if (objects[obj].explosion_rate >= EXPLOSION_DESPAWN) {
				remove_point_light(obj);
				objects[obj].is_deleted = true;
			}
		}
...
std::vector<GLfloat> get_vert_repetitive(int max_distance) {
	std::vector<GLfloat> result;
	for (int i = -max_distance; i < max_distance; ++i) {
		for (int j = -max_distance; j < max_distance; ++j) {
			result.push_back(i+1); result.push_back(j); result.push_back(-1);
			result.push_back(i); result.push_back(j); result.push_back(-1);
			result.push_back(i+1); result.push_back(j+1); result.push_back(-1);
			result.push_back(i+1); result.push_back(j+1); result.push_back(-1);
			result.push_back(i); result.push_back(j); result.push_back(-1);
			result.push_back(i); result.push_back(j+1); result.push_back(-1);
		}
	}
	return result;
}

std::vector<GLfloat> get_uv_repetitive(const std::vector<GLfloat>& vert) {
	std::vector<GLfloat> result;
...
int main(int argc, char* argv[])
{
	...
	size_t texture_uv_floor = myScene.load_texture("textures/stone_road.bmp");
	...
	// floor
	std::vector<GLfloat> floor_vert = get_vert_repetitive(myScene.projection_dist_max / FLOOR_SCALE);
	std::vector<GLfloat> floor_uv = get_uv_repetitive(pyramid_enemy_vert);
	MyObjectRaw floor_raw(floor_vert, false, 0);
	floor_raw.set_texture(texture_uv_floor, "textureSamplerFireball", floor_uv, shader_texture_basic);
	...
	size_t floor_first = myScene.add_object(floor_raw, TYPE_FLOOR);
	myScene[floor_first].scale = {FLOOR_SCALE, FLOOR_SCALE, FLOOR_SCALE};
	...
	do {
		float delta_time = (current_ms() - last_frame_time) * time_speed;
		...
		float EXPLOSION_SPEED_TIMED = glm::pow(EXPLOSION_SPEED, delta_time / 1000);
		...
		for (size_t obj = 0; obj < myScene.amount_of_objects(); ++obj) {
			if (myScene[obj].label == TYPE_SKY) {
				...
				fireball_hit = obj;
			} else if (myScene[obj].label == TYPE_FLOOR) {
				floor = obj;
		...
		while (fireball != sphere_fireballs.end()) {
			...
			while (enemy != pyramid_enemies.end()) {
				...
				if (dist < coll) {
					myScene[*fireball].explosion_rate = EXPLOSION_SPEED_TIMED;
					myScene[*enemy].explosion_rate = EXPLOSION_SPEED_TIMED;
					break;
				}
				++enemy;
			}
		...
		// create objects
		if (current_ms() - enemy_last_spawn_time >= ENEMY_SPAWN_DELTA / time_speed) {
		...
		if (glfwGetKey(window, KEY_FIREBALL) == GLFW_PRESS) {
			if (current_ms() - fireball_last_spawn_time >= FIREBALL_COOLDOWN / time_speed) {
		...
		myScene[floor].translation = {myScene.camera_pos[0] - mod_float(myScene.camera_pos[0], FLOOR_SCALE),
									  myScene.camera_pos[1] - mod_float(myScene.camera_pos[1], FLOOR_SCALE),
									  -1};
		...
		if (glfwGetKey(window, KEY_INCREASE_TIME_SPEED) == GLFW_PRESS) {
			time_speed *= TIME_SPEED_INCREASE_SPEED;
		}
		if (glfwGetKey(window, KEY_DECREASE_TIME_SPEED) == GLFW_PRESS) {
			time_speed /= TIME_SPEED_INCREASE_SPEED;
		}
